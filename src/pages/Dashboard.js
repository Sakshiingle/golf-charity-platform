...
function Dashboard() {
  ...
  const handleAddScore = async () => {
    if (!newScore || !newDate) { setMessage('Please enter score and date'); return; }
        ...
        {['overview', 'scores', 'charity', 'subscription'].map((tab) => (
          <button
            ...
            className={`px-4 py-2 capitalize mr-2 font-medium border-b-2 transition ${
              activeTab === tab ? 'border-green-400 text-green-400' : 'border-transparent text-gray-400'
            }`}>
            {tab}
          </button>
        ))}
        ...
        {activeTab === 'overview' && (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="bg-gray-800 p-6 rounded-lg">
              ...
              <p className={`text-2xl font-bold mt-1 ${profile?.subscription_status === 'active' ? 'text-green-400' : 'text-red-400'}`}>
                {profile?.subscription_status?.toUpperCase() || 'INACTIVE'}
              </p>
              <p className="text-gray-500 text-sm mt-1">{profile?.subscription_plan || 'No plan'}</p>
            </div>
            <div className="bg-gray-800 p-6 rounded-lg">
              <p className="text-gray-400 text-sm">Scores Entered</p>
              <p className="text-2xl font-bold mt-1 text-green-400">{scores.length} / 5</p>
              <p className="text-gray-500 text-sm mt-1">Last 5 scores kept</p>
            </div>
            <div className="bg-gray-800 p-6 rounded-lg">
              <p className="text-gray-400 text-sm">Charity Contribution</p>
              <p className="text-2xl font-bold mt-1 text-green-400">{profile?.charity_percentage || 10}%</p>
              <p className="text-gray-500 text-sm mt-1">of subscription fee</p>
            </div>
          </div>
        )}
        ...
        {activeTab === 'scores' && (
          <div>
            ...
            <div className="bg-gray-800 p-6 rounded-lg mb-6">
              ...
              <p className="text-gray-400 text-sm mt-2">Only last 5 scores are kept. Oldest is removed automatically.</p>
            </div>
            ...
            <div className="bg-gray-800 rounded-lg overflow-hidden">
              <table className="w-full">
                ...
                <tbody>
                  {scores.length === 0 ? (
                    <tr><td colSpan="3" className="px-6 py-8 text-center text-gray-400">No scores yet. Add your first score!</td></tr>
                  ) : (
                    scores.map((s, i) => (
                      <tr key={s.id} className="border-t border-gray-700">
                        <td className="px-6 py-4 text-gray-400">{i + 1}</td>
                        <td className="px-6 py-4 text-green-400 font-bold">{s.score}</td>
                        <td className="px-6 py-4 text-gray-300">{s.score_date}</td>
                      </tr>
                    ))
                  )}
                </tbody>
              ...
              {charities.map((charity) => (
                <div
                  ...
                  className={`bg-gray-800 p-6 rounded-lg border-2 cursor-pointer transition ${
                    profile?.charity_id === charity.id ? 'border-green-400' : 'border-gray-700 hover:border-gray-500'
                  }`}
                  onClick={() => handleSelectCharity(charity.id)}>
                  <img src={charity.image_url} alt={charity.name} className="w-full h-32 object-cover rounded mb-4" />
                  ...
                  {profile?.charity_id === charity.id && (
                    <p className="text-green-400 text-sm font-bold mt-3">✓ Selected</p>
                  )}
                </div>
              ))}
            </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="bg-gray-800 p-8 rounded-lg border border-gray-700">
                ...
                <p className="text-4xl font-bold text-green-400 mb-4">₹999<span className="text-lg text-gray-400">/mo</span></p>
                ...
                <button
                  ...
                  {profile?.subscription_plan === 'monthly' ? '✓ Current Plan' : 'Subscribe Monthly'}
                </button>
              </div>
              <div className="bg-gray-800 p-8 rounded-lg border border-green-400">
                <div className="text-green-400 text-sm font-bold mb-2">BEST VALUE</div>
                <h4 className="text-2xl font-bold mb-2">Yearly</h4>
                <p className="text-4xl font-bold text-green-400 mb-4">₹8999<span className="text-lg text-gray-400">/yr</span></p>
                <ul className="text-gray-300 mb-6 space-y-2">
                ...
                <button
                  ...
                  className="w-full py-3 bg-green-500 rounded-lg font-bold hover:bg-green-600">
                  {profile?.subscription_plan === 'yearly' ? '✓ Current Plan' : 'Subscribe Yearly'}
                </button>
              </div>
            </div>
          </div>
